...
class Crawler:
    ...
    def get_image(self, word=''):
        ...
        while pagenum <= self.__amount: # 当开始页码小于总页码时循环
            url = 'https://api.pixivic.com/illustrations?keyword='+search+'&page='+str(pagenum)   # 写入rul
            print(url)  # 打印url
            try:
    ...
    def start(self, word, page_num, start_page):
...
if __name__ == "__main__":
    word = input('你想要下载谁的涩图?请输入:')
    crawler = Crawler(1.3, word)    # 新建一个对象, 并初始化传入延时的时间, 以及想要查找的字段
    crawler.start(word, 10, 1)      # 总页码10, 开始页码1